...
async function main() {
  ...
  if (match) {
    ...
    // Try to construct one from environment variables
    const geminiKvId = process.env.GEMINI_CLI_KV_ID || process.env.GEMINI_CLI_KV_NAMESPACE_ID || process.env.GEMINI_CLI_KV;
    if (geminiKvId) {
      const kvBlock = [
        'kv_namespaces = [',
        `  { binding = "GEMINI_CLI_KV", id = "${geminiKvId}" }`,
        ']\n',
      ].join('\n');
      // Insert after compatibility_flags or at the end
      const insertAfter = /(compatibility_flags\s*=\s*\[[^\]]*\][^\n]*\n)/m;
      if (insertAfter.test(updated)) {
        updated = updated.replace(insertAfter, `$1\n# --- KV Namespaces (generated) ---\n${kvBlock}\n`);
      } else {
        updated += `\n# --- KV Namespaces (generated) ---\n${kvBlock}\n`;
      }
      log('Injected kv_namespaces block using GEMINI_CLI_KV_ID from environment');
    } else {
      log('WARN: GEMINI_CLI_KV_ID not set; kv_namespaces will not be present in generated config');
    }
  }
